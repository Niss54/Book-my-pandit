...
Deno.serve(async (req: Request) => {
  ...
  if (panditId == null || panditId.isEmpty || idempotencyKey == null || idempotencyKey.isEmpty) {
    return new Response(JSON.stringify({ error: 'pandit_id and idempotency_key are required' }), {
      status: 400,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }
  ...
  if (panditResult.error != null || panditResult.data == null) {
    return new Response(JSON.stringify({ error: 'Pandit not found' }), {
      status: 404,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }
  ...
  if (panditResult.data.is_active !== true) {
    return new Response(JSON.stringify({ error: 'Pandit is not active' }), {
      status: 400,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }
